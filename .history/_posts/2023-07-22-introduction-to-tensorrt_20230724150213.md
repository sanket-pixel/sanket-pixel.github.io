...
```
In this part, we allocate device memory on the GPU for storing the input and output data during inference using `cuda.mem_alloc()` The size of the memory buffers is determined by the size of the page-locked memory buffers `h_input` and `h_output` that we created earlier.
